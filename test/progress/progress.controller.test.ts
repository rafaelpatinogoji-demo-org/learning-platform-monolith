...
describe('progressController', () => {
  ...
  describe('markComplete', () => {
    it('should mark lesson progress successfully', async () => {
      ...
      mockReq.body = { enrollmentId, lessonId, completed };
    ...
    it('should return 404 when enrollment not found', async () => {
      mockReq.body = { enrollmentId: 999, lessonId: 1, completed: true };
      mockReq.user = { id: 1, email: 'student@example.com', role: 'student' };
    ...
    it('should return 403 when user cannot mark progress for enrollment', async () => {
      mockReq.body = { enrollmentId: 1, lessonId: 1, completed: true };
      mockReq.user = { id: 1, email: 'student@example.com', role: 'student' };
  ...
  describe('getMyProgress', () => {
    it('should return user progress for course', async () => {
      ...
      mockReq.query = { courseId: courseId.toString() };
      mockReq.user = { id: userId, email: 'student@example.com', role: 'student' };
      ...
      expect(mockProgressService.getUserCourseProgress).toHaveBeenCalledWith(userId, courseId);
    });
  ...
  describe('getCourseProgress', () => {
    it('should return course progress for instructor', async () => {
      ...
      expect(mockRes.json).toHaveBeenCalledWith({
        ok: true,
        data: mockProgress,
        count: mockProgress.length,
        version: expect.any(String)
      });
      expect(mockProgressService.getCourseProgress).toHaveBeenCalledWith(courseId, userId, 'instructor');
    });
